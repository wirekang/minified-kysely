...
declare const OPERATORS: readonly ["=", "==", "!=", "<>", ">", ">=", "<", "<=", "in", "not in", "is", "is not", "like", "not like", "match", "ilike", "not ilike", "@>", "<@", "&&", "?", "?&", "!<", "!>", "<=>", "!~", "~", "~*", "!~*", "@@", "@@@", "!!", "<->", "regexp", "+", "-", "*", "/", "%", "^", "&", "|", "#", "<<", ">>", "&&", "||", "->", "->>", "not", "-", "exists", "not exists", "between", "between symmetric"];
...
declare class ExpressionWrapper<DB, TB extends keyof DB, T> implements AliasableExpression<T> {
    ...
    or(expression: OperandExpression<SqlBool>): T extends SqlBool ? OrWrapper<DB, TB, SqlBool> : KyselyTypeError<'or() method can only be called on boolean expressions'>;
    /**
    ...
    and(expression: OperandExpression<SqlBool>): T extends SqlBool ? AndWrapper<DB, TB, SqlBool> : KyselyTypeError<'and() method can only be called on boolean expressions'>;
    /**
...
declare class AggregateFunctionBuilder<DB, TB extends keyof DB, O = unknown> implements AliasableExpression<O> {
    ...
    filterWhere(expression: ExpressionOrFactory<DB, TB, SqlBool>): AggregateFunctionBuilder<DB, TB, O>;
    /**
...
interface FunctionModule<DB, TB extends keyof DB> {
    ...
    any<T>(subquery: SelectQueryBuilder<any, any, Record<string, T>>): ExpressionWrapper<DB, TB, T>;
...
type Selection<DB, TB extends keyof DB, SE> = DrainOuterGeneric<{
...
type AllSelection<DB, TB extends keyof DB> = DrainOuterGeneric<Selectable<{
...
interface ExpressionBuilder<DB, TB extends keyof DB> {
     ...
     */
    selectNoFrom<SE extends SelectExpression<DB, TB>>(selection: SelectArg<DB, TB, SE>): SelectQueryBuilder<DB, TB, Selection<DB, TB, SE>>;
    /**
     ...
     * ```
     */
    between<RE extends ReferenceExpression<DB, TB>>(expr: RE, start: OperandValueExpression<DB, TB, RE>, end: OperandValueExpression<DB, TB, RE>): ExpressionWrapper<DB, TB, SqlBool>;
    /**
     ...
     * ```
     */
    betweenSymmetric<RE extends ReferenceExpression<DB, TB>>(expr: RE, start: OperandValueExpression<DB, TB, RE>, end: OperandValueExpression<DB, TB, RE>): ExpressionWrapper<DB, TB, SqlBool>;
    /**
     ...
     * ```
     */
    and(exprs: ReadonlyArray<OperandExpression<SqlBool>>): ExpressionWrapper<DB, TB, SqlBool>;
     ...
     * ```
     */
    or(exprs: ReadonlyArray<OperandExpression<SqlBool>>): ExpressionWrapper<DB, TB, SqlBool>;
 ...
 */
type OperandExpression<V> = Expression<V> | SelectQueryBuilder<any, any, Record<string, V>>;
...
interface WhereInterface<DB, TB extends keyof DB> {
    ...
    where(expression: ExpressionOrFactory<DB, TB, SqlBool>): WhereInterface<DB, TB>;
    /**
...
interface HavingInterface<DB, TB extends keyof DB> {
    ...
    having(expression: ExpressionOrFactory<DB, TB, SqlBool>): HavingInterface<DB, TB>;
    /**
...
interface SelectQueryBuilder<DB, TB extends keyof DB, O> extends WhereInterface<DB, TB>, HavingInterface<DB, TB>, AliasableExpression<O>, Compilable<O>, Explainable, Streamable<O> {
    /**
     * Adds a `where` expression to the query.
     *
     * Calling this method multiple times will combine the expressions using `and`.
    ...
    where(expression: ExpressionOrFactory<DB, TB, SqlBool>): SelectQueryBuilder<DB, TB, O>;
    /**
     * Adds a `where` clause where both sides of the operator are references
     * to columns.
     *
    ...
    having(expression: ExpressionOrFactory<DB, TB, SqlBool>): SelectQueryBuilder<DB, TB, O>;
    /**
     * Just like {@link WhereInterface.whereRef | whereRef} but adds a `having` statement
     * instead of a `where` statement.
     */
     ...
     * ```
     */
    orderBy(orderBy: UndirectedOrderByExpression<DB, TB, O>, direction?: OrderByDirectionExpression): SelectQueryBuilder<DB, TB, O>;
    orderBy(ref: DirectedOrderByStringReference<DB, TB, O>): SelectQueryBuilder<DB, TB, O>;
    orderBy(refs: ReadonlyArray<OrderByExpression<DB, TB, O>>): SelectQueryBuilder<DB, TB, O>;
    /**
...
interface AliasedSelectQueryBuilder<DB, TB extends keyof DB, O = undefined, A extends string = never> extends AliasedExpression<O, A> {
 ...
 */
type ExtractColumnType<DB, TB extends keyof DB, C> = DrainOuterGeneric<{
    [T in TB]: C extends keyof DB[T] ? DB[T][C] : never;
}[TB]>;
/**
 * Given a database type and a union of table names in that db, returns
 ...
 * ```
 */
type AnyColumnWithTable<DB, TB extends keyof DB> = DrainOuterGeneric<{
 ...
 */
type AnyAliasedColumn<DB, TB extends keyof DB> = DrainOuterGeneric<{
 ...
 */
type AnyAliasedColumnWithTable<DB, TB extends keyof DB> = DrainOuterGeneric<{
...
type NarrowPartial<S, T> = DrainOuterGeneric<T extends object ? {
...
declare class OnConflictBuilder<DB, TB extends keyof DB> implements WhereInterface<DB, TB> {
     ...
     * ```
     */
    where(expression: ExpressionOrFactory<DB, TB, SqlBool>): OnConflictBuilder<DB, TB>;
    /**
     * Specify an index predicate for the index target.
     *
...
declare class OnConflictUpdateBuilder<DB, TB extends keyof DB> implements WhereInterface<DB, TB>, OperationNodeSource {
     ...
     * ```
     */
    where(expression: ExpressionOrFactory<DB, TB, SqlBool>): OnConflictUpdateBuilder<DB, TB>;
    /**
     * Specify a where condition for the update operation.
     *
     * See {@link WhereInterface.whereRef} for more info.
...
declare class DeleteQueryBuilder<DB, TB extends keyof DB, O> implements WhereInterface<DB, TB>, ReturningInterface<DB, TB, O>, OperationNodeSource, Compilable<O>, Explainable, Streamable<O> {
    ...
    where(expression: ExpressionOrFactory<DB, TB, SqlBool>): DeleteQueryBuilder<DB, TB, O>;
    /**
     * Adds a `where` clause where both sides of the operator are references
     * to columns.
     *
...
declare class QueryCreator<DB> {
     ...
     * ```
     */
    selectNoFrom<SE extends SelectExpression<DB, never>>(selection: SelectArg<DB, never, SE>): SelectQueryBuilder<DB, never, Selection<DB, never, SE>>;
    /**
...
export { HavingNode as $, AliasNode as A, CreateIndexNode as B, CompiledQuery as C, type DialectAdapter as D, DropIndexNode as E, FromNode as F, GroupByNode as G, type PrimaryKeyConstraintNode as H, InsertQueryNode as I, JoinNode as J, type KyselyPlugin as K, LimitNode as L, UniqueConstraintNode as M, ReferencesNode as N, type OperationNode as O, PrimitiveValueListNode as P, type QueryExecutor as Q, type RawBuilder as R, SelectQueryNode as S, TableNode as T, UpdateQueryNode as U, ValueListNode as V, WhereNode as W, CheckConstraintNode as X, WithNode as Y, CommonTableExpressionNode as Z, CommonTableExpressionNameNode as _, type RootOperationNode as a, type Simplify as a$, CreateSchemaNode as a0, DropSchemaNode as a1, AlterTableNode as a2, DropColumnNode as a3, RenameColumnNode as a4, AlterColumnNode as a5, ModifyColumnNode as a6, AddConstraintNode as a7, DropConstraintNode as a8, ForeignKeyConstraintNode as a9, JSONOperatorChainNode as aA, type Driver as aB, Kysely as aC, type MigrationLockOptions as aD, type Dialect as aE, type DatabaseIntrospector as aF, type SchemaMetadata as aG, type DatabaseMetadataOptions as aH, type TableMetadata as aI, type DatabaseMetadata as aJ, type TransactionSettings as aK, type PluginTransformQueryArgs as aL, type PluginTransformResultArgs as aM, type UnknownRow as aN, type Compilable as aO, InsertResult as aP, UpdateResult as aQ, DeleteResult as aR, type ExpressionOrFactory as aS, type ExpressionBuilder as aT, expressionBuilder as aU, type AnyAliasedColumn as aV, type AnyAliasedColumnWithTable as aW, type AnyColumn as aX, type AnyColumnWithTable as aY, type AnySelectQueryBuilder as aZ, type Equals as a_, DataTypeNode as aa, SelectAllNode as ab, IdentifierNode as ac, SchemableIdentifierNode as ad, ValueNode as ae, OperatorNode as af, CreateViewNode as ag, DropViewNode as ah, GeneratedNode as ai, DefaultValueNode as aj, OnNode as ak, SelectModifierNode as al, CreateTypeNode as am, DropTypeNode as an, ExplainNode as ao, AggregateFunctionNode as ap, OverNode as aq, PartitionByNode as ar, PartitionByItemNode as as, SetOperationNode as at, UsingNode as au, CaseNode as av, WhenNode as aw, JSONReferenceNode as ax, JSONPathNode as ay, JSONPathLegNode as az, type QueryId as b, type DeleteQueryBuilderWithInnerJoin as b$, type SqlBool as b0, type SelectExpression as b1, type SelectArg as b2, type Selection as b3, type ReferenceExpression as b4, type ReferenceExpressionOrList as b5, type SimpleReferenceExpression as b6, type StringReference as b7, type ExtractTypeFromStringReference as b8, type ExtractTypeFromReferenceExpression as b9, isExpression as bA, isAliasedExpression as bB, ExpressionWrapper as bC, AliasedExpressionWrapper as bD, OrWrapper as bE, AndWrapper as bF, type WhereInterface as bG, type ReturningInterface as bH, type HavingInterface as bI, type SelectQueryBuilder as bJ, createSelectQueryBuilder as bK, type SelectQueryBuilderProps as bL, type AliasedSelectQueryBuilder as bM, type SelectQueryBuilderWithInnerJoin as bN, type SelectQueryBuilderWithLeftJoin as bO, type SelectQueryBuilderWithRightJoin as bP, type SelectQueryBuilderWithFullJoin as bQ, InsertQueryBuilder as bR, type InsertQueryBuilderProps as bS, UpdateQueryBuilder as bT, type UpdateQueryBuilderProps as bU, type UpdateQueryBuilderWithInnerJoin as bV, type UpdateQueryBuilderWithLeftJoin as bW, type UpdateQueryBuilderWithRightJoin as bX, type UpdateQueryBuilderWithFullJoin as bY, DeleteQueryBuilder as bZ, type DeleteQueryBuilderProps as b_, type ValueExpression as ba, type ValueExpressionOrList as bb, type TableExpression as bc, type TableExpressionOrList as bd, type JoinReferenceExpression as be, type JoinCallbackExpression as bf, type InsertObject as bg, type UpdateObject as bh, type OrderByExpression as bi, type OrderByDirectionExpression as bj, type ComparisonOperatorExpression as bk, type OperandValueExpression as bl, type OperandValueExpressionOrList as bm, type FilterObject as bn, type OperandExpression as bo, Transaction as bp, type KyselyProps as bq, isKyselyProps as br, type KyselyConfig as bs, ConnectionBuilder as bt, TransactionBuilder as bu, QueryCreator as bv, type QueryCreatorProps as bw, type Expression as bx, type AliasableExpression as by, type AliasedExpression as bz, type DatabaseConnection as c, AlterColumnBuilder as c$, type DeleteQueryBuilderWithLeftJoin as c0, type DeleteQueryBuilderWithRightJoin as c1, type DeleteQueryBuilderWithFullJoin as c2, type NoResultErrorConstructor as c3, NoResultError as c4, isNoResultErrorConstructor as c5, JoinBuilder as c6, type JoinBuilderProps as c7, type FunctionModule as c8, createFunctionModule as c9, CreateTypeBuilder as cA, type CreateTypeBuilderProps as cB, DropTableBuilder as cC, type DropTableBuilderProps as cD, DropTypeBuilder as cE, type DropTypeBuilderProps as cF, CreateIndexBuilder as cG, type CreateIndexBuilderProps as cH, DropIndexBuilder as cI, type DropIndexBuilderProps as cJ, CreateSchemaBuilder as cK, type CreateSchemaBuilderProps as cL, DropSchemaBuilder as cM, type DropSchemaBuilderProps as cN, ColumnDefinitionBuilder as cO, type ColumnDefinitionBuilderCallback as cP, type ForeignKeyConstraintBuilderInterface as cQ, ForeignKeyConstraintBuilder as cR, AlterTableBuilder as cS, type AlterTableBuilderProps as cT, type ColumnAlteringInterface as cU, AlterTableColumnAlteringBuilder as cV, type AlterTableColumnAlteringBuilderProps as cW, CreateViewBuilder as cX, type CreateViewBuilderProps as cY, DropViewBuilder as cZ, type DropViewBuilderProps as c_, OnConflictBuilder as ca, type OnConflictBuilderProps as cb, type OnConflictDatabase as cc, type OnConflictTables as cd, OnConflictDoNothingBuilder as ce, OnConflictUpdateBuilder as cf, AggregateFunctionBuilder as cg, AliasedAggregateFunctionBuilder as ch, type AggregateFunctionBuilderProps as ci, type OverBuilderCallback as cj, CaseBuilder as ck, CaseThenBuilder as cl, CaseWhenBuilder as cm, CaseEndBuilder as cn, JSONPathBuilder as co, TraversedJSONPathBuilder as cp, AliasedJSONPathBuilder as cq, type RawBuilderProps as cr, createRawBuilder as cs, type AliasedRawBuilder as ct, type QueryExecutorProvider as cu, SchemaModule as cv, CreateTableBuilder as cw, type CreateTableBuilderProps as cx, type ColumnBuilderCallback as cy, type ForeignKeyConstraintBuilderCallback as cz, type QueryResult as d, type SetOperator as d$, AlteredColumnBuilder as d0, type AlterColumnBuilderCallback as d1, DynamicModule as d2, TRANSACTION_ISOLATION_LEVELS as d3, type IsolationLevel as d4, type ColumnMetadata as d5, type AlterColumnNodeProps as d6, type AlterTableNodeTableProps as d7, type AlterTableColumnAlterationNode as d8, type ColumnDefinitionNodeProps as d9, type OperationNodeKind as dA, COMPARISON_OPERATORS as dB, ARITHMETIC_OPERATORS as dC, JSON_OPERATORS as dD, BINARY_OPERATORS as dE, UNARY_FILTER_OPERATORS as dF, UNARY_OPERATORS as dG, OPERATORS as dH, type ComparisonOperator as dI, type ArithmeticOperator as dJ, type JSONOperator as dK, type JSONOperatorWith$ as dL, type BinaryOperator as dM, type UnaryOperator as dN, type UnaryFilterOperator as dO, type Operator as dP, isOperator as dQ, isBinaryOperator as dR, isComparisonOperator as dS, isArithmeticOperator as dT, isJSONOperator as dU, PrimaryConstraintNode as dV, QueryNode as dW, ON_MODIFY_FOREIGN_ACTIONS as dX, type OnModifyForeignAction as dY, type UpdateValuesNode as dZ, type WithNodeParams as d_, type ConstraintNode as da, type CreateIndexNodeProps as db, type IndexType as dc, type CreateSchemaNodeParams as dd, ON_COMMIT_ACTIONS as de, type OnCommitAction as df, type CreateTableNodeParams as dg, type CreateTypeNodeParams as dh, type CreateViewNodeParams as di, type ColumnDataType as dj, type DataTypeParams as dk, type DropConstraintNodeProps as dl, type DropIndexNodeProps as dm, type DropSchemaNodeParams as dn, type DropTablexNodeParams as dp, type DropTypeNodeParams as dq, type DropViewNodeParams as dr, type ForeignKeyConstraintNodeProps as ds, type GeneratedNodeParams as dt, type InsertQueryNodeProps as du, type JoinType as dv, type OnConflictNodeProps as dw, type OnDuplicateKeyNodeProps as dx, type OperationNodeSource as dy, isOperationNodeSource as dz, type ConnectionProvider as e, type JSONPathLegType as e0, type ColumnType as e1, type Generated as e2, type GeneratedAlways as e3, type JSONColumnType as e4, type SelectType as e5, type InsertType as e6, type UpdateType as e7, type NullableInsertKeys as e8, type NonNullableInsertKeys as e9, type UpdateKeys as ea, type Selectable as eb, type Insertable as ec, type Updateable as ed, isCompilable as ee, type ExplainFormat as ef, type Explainable as eg, type Streamable as eh, LOG_LEVELS as ei, type LogLevel as ej, type QueryLogEvent as ek, type ErrorLogEvent as el, type LogEvent as em, type Logger as en, type LogConfig as eo, Log as ep, type QueryCompiler as f, SelectionNode as g, ColumnNode as h, ReferenceNode as i, AndNode as j, OrNode as k, ParensNode as l, RawNode as m, DeleteQueryNode as n, ReturningNode as o, CreateTableNode as p, AddColumnNode as q, ColumnDefinitionNode as r, DropTableNode as s, OrderByNode as t, OrderByItemNode as u, GroupByItemNode as v, ColumnUpdateNode as w, OffsetNode as x, OnConflictNode as y, OnDuplicateKeyNode as z };
