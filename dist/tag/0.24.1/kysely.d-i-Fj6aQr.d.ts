...
type WhereExpressionFactory<DB, TB extends keyof DB> = (eb: WhereExpressionBuilder<DB, TB>) => Expression<SqlBool> | WhereExpressionBuilder<DB, TB>;
...
type OutputBoundStringReference<DB, TB extends keyof DB, C extends StringReference<DB, TB>, O> = IsNever<O> extends true ? C : Equals<ExtractTypeFromReferenceExpression<DB, TB, C> | null, O | null> extends true ? C : never;
type StringReferenceBoundAggregateFunctionBuilder<DB, TB extends keyof DB, C extends StringReference<DB, TB>, O> = AggregateFunctionBuilder<DB, TB, ExtractTypeFromReferenceExpression<DB, TB, C> | (null extends O ? null : never)>;
...
type ExpressionOrFactory<DB, TB extends keyof DB, V> = SelectQueryBuilder<any, any, Record<string, V>> | SelectQueryBuilderFactory<DB, TB, Record<string, V>> | Expression<V> | ExpressionFactory<DB, TB, V>;
...
type FromTables<DB, TB extends keyof DB, TE> = TB | ExtractAliasFromTableExpression<DB, TE>;
type ExtractTableAlias<DB, TE> = TE extends `${string} as ${infer TA}` ? TA : TE extends keyof DB ? TE : never;
type PickTableWithAlias<DB, T extends AnyAliasedTable<DB>> = T extends `${infer TB} as ${infer A}` ? TB extends keyof DB ? Record<A, DB[TB]> : never : never;
...
type OnExpressionFactory<DB, TB extends keyof DB> = (eb: OnExpressionBuilder<DB, TB>) => Expression<SqlBool> | OnExpressionBuilder<DB, TB>;
...
type HavingExpressionFactory<DB, TB extends keyof DB> = (eb: HavingExpressionBuilder<DB, TB>) => Expression<SqlBool> | HavingExpressionBuilder<DB, TB>;
...
type InnerJoinedBuilder$2<DB, TB extends keyof DB, O, A extends string, R> = A extends keyof DB ? SelectQueryBuilder<InnerJoinedDB$2<DB, A, R>, TB | A, O> : SelectQueryBuilder<DB & Record<A, R>, TB | A, O>;
type InnerJoinedDB$2<DB, A extends string, R> = {
...
type LeftJoinedBuilder$2<DB, TB extends keyof DB, O, A extends keyof any, R> = A extends keyof DB ? SelectQueryBuilder<LeftJoinedDB$2<DB, A, R>, TB | A, O> : SelectQueryBuilder<DB & Record<A, Nullable<R>>, TB | A, O>;
type LeftJoinedDB$2<DB, A extends keyof any, R> = {
...
type RightJoinedDB$2<DB, TB extends keyof DB, A extends keyof any, R> = {
...
type OuterJoinedBuilderDB$2<DB, TB extends keyof DB, A extends keyof any, R> = {
...
type InnerJoinedBuilder$1<DB, TB extends keyof DB, O, A extends string, R> = A extends keyof DB ? DeleteQueryBuilder<InnerJoinedDB$1<DB, A, R>, TB | A, O> : DeleteQueryBuilder<DB & Record<A, R>, TB | A, O>;
type InnerJoinedDB$1<DB, A extends string, R> = {
    [C in keyof DB | A]: C extends A ? R : C extends keyof DB ? DB[C] : never;
};
...
type LeftJoinedBuilder$1<DB, TB extends keyof DB, O, A extends keyof any, R> = A extends keyof DB ? DeleteQueryBuilder<LeftJoinedDB$1<DB, A, R>, TB | A, O> : DeleteQueryBuilder<DB & Record<A, Nullable<R>>, TB | A, O>;
type LeftJoinedDB$1<DB, A extends keyof any, R> = {
    [C in keyof DB | A]: C extends A ? Nullable<R> : C extends keyof DB ? DB[C] : never;
};
...
type RightJoinedDB$1<DB, TB extends keyof DB, A extends keyof any, R> = {
    [C in keyof DB | A]: C extends A ? R : C extends TB ? Nullable<DB[C]> : C extends keyof DB ? DB[C] : never;
};
...
type OuterJoinedBuilderDB$1<DB, TB extends keyof DB, A extends keyof any, R> = {
    [C in keyof DB | A]: C extends A ? Nullable<R> : C extends TB ? Nullable<DB[C]> : C extends keyof DB ? DB[C] : never;
};
...
type InnerJoinedBuilder<DB, UT extends keyof DB, TB extends keyof DB, O, A extends string, R> = A extends keyof DB ? UpdateQueryBuilder<InnerJoinedDB<DB, A, R>, UT, TB | A, O> : UpdateQueryBuilder<DB & Record<A, R>, UT, TB | A, O>;
...
type LeftJoinedBuilder<DB, UT extends keyof DB, TB extends keyof DB, O, A extends keyof any, R> = A extends keyof DB ? UpdateQueryBuilder<LeftJoinedDB<DB, A, R>, UT, TB | A, O> : UpdateQueryBuilder<DB & Record<A, Nullable<R>>, UT, TB | A, O>;
...
type RecursiveCommonTableExpression<DB, CN extends string> = (creator: QueryCreator<DB & Record<ExtractTableFromCommonTableExpressionName<CN>, ExtractRowFromCommonTableExpressionName<CN>>>) => CommonTableExpressionOutput<DB, CN>;
type QueryCreatorWithCommonTableExpression<DB, CN extends string, CTE> = QueryCreator<DB & Record<ExtractTableFromCommonTableExpressionName<CN>, ExtractRowFromCommonTableExpression<CTE>>>;
...
export { AlterTableNode as $, AliasNode as A, UniqueConstraintNode as B, CompiledQuery as C, type DialectAdapter as D, ReferencesNode as E, FromNode as F, GroupByNode as G, CheckConstraintNode as H, InsertQueryNode as I, JoinNode as J, type KyselyPlugin as K, LimitNode as L, WithNode as M, CommonTableExpressionNode as N, type OperationNode as O, PrimitiveValueListNode as P, type QueryExecutor as Q, RawBuilder as R, SelectQueryNode as S, TableNode as T, UpdateQueryNode as U, ValueListNode as V, WhereNode as W, CommonTableExpressionNameNode as X, HavingNode as Y, CreateSchemaNode as Z, DropSchemaNode as _, type RootOperationNode as a, type ValueExpression as a$, DropColumnNode as a0, RenameColumnNode as a1, AlterColumnNode as a2, ModifyColumnNode as a3, AddConstraintNode as a4, DropConstraintNode as a5, ForeignKeyConstraintNode as a6, DataTypeNode as a7, SelectAllNode as a8, IdentifierNode as a9, type TransactionSettings as aA, type PluginTransformQueryArgs as aB, type PluginTransformResultArgs as aC, type UnknownRow as aD, type Compilable as aE, InsertResult as aF, UpdateResult as aG, DeleteResult as aH, type ExpressionBuilder as aI, expressionBuilder as aJ, type AnyAliasedColumn as aK, type AnyAliasedColumnWithTable as aL, type AnyColumn as aM, type AnyColumnWithTable as aN, type AnySelectQueryBuilder as aO, type Equals as aP, type Simplify as aQ, type SqlBool as aR, type SelectExpression as aS, type SelectArg as aT, type Selection as aU, type ReferenceExpression as aV, type ReferenceExpressionOrList as aW, type SimpleReferenceExpression as aX, type StringReference as aY, type ExtractTypeFromStringReference as aZ, type ExtractTypeFromReferenceExpression as a_, SchemableIdentifierNode as aa, ValueNode as ab, OperatorNode as ac, CreateViewNode as ad, DropViewNode as ae, GeneratedNode as af, DefaultValueNode as ag, OnNode as ah, SelectModifierNode as ai, CreateTypeNode as aj, DropTypeNode as ak, ExplainNode as al, AggregateFunctionNode as am, OverNode as an, PartitionByNode as ao, PartitionByItemNode as ap, SetOperationNode as aq, UsingNode as ar, type Driver as as, Kysely as at, type Dialect as au, type DatabaseIntrospector as av, type SchemaMetadata as aw, type DatabaseMetadataOptions as ax, type TableMetadata as ay, type DatabaseMetadata as az, type QueryId as b, OnConflictUpdateBuilder as b$, type ValueExpressionOrList as b0, type TableExpression as b1, type TableExpressionOrList as b2, type JoinReferenceExpression as b3, type JoinCallbackExpression as b4, type InsertObject as b5, type UpdateObject as b6, type OrderByExpression as b7, type OrderByDirectionExpression as b8, type ComparisonOperatorExpression as b9, type SelectQueryBuilderWithFullJoin as bA, InsertQueryBuilder as bB, type InsertQueryBuilderProps as bC, UpdateQueryBuilder as bD, type UpdateQueryBuilderProps as bE, type UpdateQueryBuilderWithInnerJoin as bF, type UpdateQueryBuilderWithLeftJoin as bG, type UpdateQueryBuilderWithRightJoin as bH, type UpdateQueryBuilderWithFullJoin as bI, DeleteQueryBuilder as bJ, type DeleteQueryBuilderProps as bK, type DeleteQueryBuilderWithInnerJoin as bL, type DeleteQueryBuilderWithLeftJoin as bM, type DeleteQueryBuilderWithRightJoin as bN, type DeleteQueryBuilderWithFullJoin as bO, type NoResultErrorConstructor as bP, NoResultError as bQ, isNoResultErrorConstructor as bR, JoinBuilder as bS, type JoinBuilderProps as bT, type FunctionModule as bU, createFunctionModule as bV, OnConflictBuilder as bW, type OnConflictBuilderProps as bX, type OnConflictDatabase as bY, type OnConflictTables as bZ, OnConflictDoNothingBuilder as b_, type OperandValueExpression as ba, type OperandValueExpressionOrList as bb, type ExistsExpression as bc, Transaction as bd, type KyselyProps as be, isKyselyProps as bf, type KyselyConfig as bg, ConnectionBuilder as bh, TransactionBuilder as bi, QueryCreator as bj, type QueryCreatorProps as bk, type Expression as bl, type AliasedExpression as bm, isExpression as bn, isAliasedExpression as bo, type WhereInterface as bp, type WhereExpressionFactory as bq, type ReturningInterface as br, type HavingInterface as bs, type HavingExpressionFactory as bt, SelectQueryBuilder as bu, type SelectQueryBuilderProps as bv, AliasedSelectQueryBuilder as bw, type SelectQueryBuilderWithInnerJoin as bx, type SelectQueryBuilderWithLeftJoin as by, type SelectQueryBuilderWithRightJoin as bz, type DatabaseConnection as c, type DropConstraintNodeProps as c$, AggregateFunctionBuilder as c0, AliasedAggregateFunctionBuilder as c1, type AggregateFunctionBuilderProps as c2, type OverBuilderCallback as c3, isRawBuilder as c4, AliasedRawBuilder as c5, type RawBuilderProps as c6, type QueryExecutorProvider as c7, SchemaModule as c8, CreateTableBuilder as c9, CreateViewBuilder as cA, type CreateViewBuilderProps as cB, DropViewBuilder as cC, type DropViewBuilderProps as cD, AlterColumnBuilder as cE, AlteredColumnBuilder as cF, type AlterColumnBuilderCallback as cG, DynamicModule as cH, TRANSACTION_ISOLATION_LEVELS as cI, type IsolationLevel as cJ, type MigrationLockOptions as cK, type ColumnMetadata as cL, type AlterColumnNodeProps as cM, type AlterTableNodeTableProps as cN, type AlterTableColumnAlterationNode as cO, type ColumnDefinitionNodeProps as cP, type ConstraintNode as cQ, type CreateIndexNodeProps as cR, type IndexType as cS, type CreateSchemaNodeParams as cT, ON_COMMIT_ACTIONS as cU, type OnCommitAction as cV, type CreateTableNodeParams as cW, type CreateTypeNodeParams as cX, type CreateViewNodeParams as cY, type ColumnDataType as cZ, type DataTypeParams as c_, type CreateTableBuilderProps as ca, type ColumnBuilderCallback as cb, type ForeignKeyConstraintBuilderCallback as cc, CreateTypeBuilder as cd, type CreateTypeBuilderProps as ce, DropTableBuilder as cf, type DropTableBuilderProps as cg, DropTypeBuilder as ch, type DropTypeBuilderProps as ci, CreateIndexBuilder as cj, type CreateIndexBuilderProps as ck, DropIndexBuilder as cl, type DropIndexBuilderProps as cm, CreateSchemaBuilder as cn, type CreateSchemaBuilderProps as co, DropSchemaBuilder as cp, type DropSchemaBuilderProps as cq, ColumnDefinitionBuilder as cr, type ColumnDefinitionBuilderCallback as cs, type ForeignKeyConstraintBuilderInterface as ct, ForeignKeyConstraintBuilder as cu, AlterTableBuilder as cv, type AlterTableBuilderProps as cw, type ColumnAlteringInterface as cx, AlterTableColumnAlteringBuilder as cy, type AlterTableColumnAlteringBuilderProps as cz, type QueryResult as d, type DropIndexNodeProps as d0, type DropSchemaNodeParams as d1, type DropTablexNodeParams as d2, type DropTypeNodeParams as d3, type DropViewNodeParams as d4, type ForeignKeyConstraintNodeProps as d5, type GeneratedNodeParams as d6, type InsertQueryNodeProps as d7, type JoinType as d8, type OnConflictNodeProps as d9, type WithNodeParams as dA, type SetOperator as dB, type ColumnType as dC, type Generated as dD, type GeneratedAlways as dE, type SelectType as dF, type InsertType as dG, type UpdateType as dH, type NullableInsertKeys as dI, type NonNullableInsertKeys as dJ, type UpdateKeys as dK, type Selectable as dL, type Insertable as dM, type Updateable as dN, isCompilable as dO, type ExplainFormat as dP, type Explainable as dQ, type Streamable as dR, LOG_LEVELS as dS, type LogLevel as dT, type QueryLogEvent as dU, type ErrorLogEvent as dV, type LogEvent as dW, type Logger as dX, type LogConfig as dY, Log as dZ, type OnDuplicateKeyNodeProps as da, type OperationNodeSource as db, isOperationNodeSource as dc, type OperationNodeKind as dd, COMPARISON_OPERATORS as de, ARITHMETIC_OPERATORS as df, BINARY_OPERATORS as dg, UNARY_FILTER_OPERATORS as dh, UNARY_OPERATORS as di, OPERATORS as dj, type ComparisonOperator as dk, type ArithmeticOperator as dl, type BinaryOperator as dm, type UnaryOperator as dn, type UnaryFilterOperator as dp, type Operator as dq, isOperator as dr, isBinaryOperator as ds, isComparisonOperator as dt, isArithmeticOperator as du, PrimaryConstraintNode as dv, QueryNode as dw, ON_MODIFY_FOREIGN_ACTIONS as dx, type OnModifyForeignAction as dy, type UpdateValuesNode as dz, type ConnectionProvider as e, type QueryCompiler as f, SelectionNode as g, ColumnNode as h, ReferenceNode as i, RawNode as j, DeleteQueryNode as k, ReturningNode as l, CreateTableNode as m, AddColumnNode as n, ColumnDefinitionNode as o, DropTableNode as p, OrderByNode as q, OrderByItemNode as r, GroupByItemNode as s, ColumnUpdateNode as t, OffsetNode as u, OnConflictNode as v, OnDuplicateKeyNode as w, CreateIndexNode as x, DropIndexNode as y, type PrimaryKeyConstraintNode as z };
